...
Xây dựng mới toàn bộ luồng khôi phục mật khẩu gồm 3 bước trên một trang duy nhất (multi-step form):
- **Bước 1 — Nhập Email**: Form nhập địa chỉ email kèm validation; khi submit tự động sinh mã OTP 6 chữ số và gửi qua EmailJS đến hộp thư người dùng.
- **Bước 2 — Xác thực OTP**: Giao diện 6 ô nhập số rời (auto-focus khi điền xong mỗi ô, Backspace quay về ô trước); đếm ngược 59 giây và nút Gửi lại OTP sau khi hết giờ.
- **Bước 3 — Đặt mật khẩu mới**: Form nhập và xác nhận mật khẩu mới với thanh đo độ mạnh thời gian thực (4 mức: Rất yếu → Rất mạnh), checklist yêu cầu (≥8 ký tự, chữ hoa, số, ký tự đặc biệt), toggle ẩn/hiện mật khẩu; gọi API backend để cập nhật mật khẩu qua Firebase Admin SDK.

Thiết kế UI premium glassmorphism: nền gradient tím, card kính mờ (backdrop-filter: blur), blob trang trí animate, lưới SVG nền, animation fade-in-up khi chuyển bước.

Tích hợp backend:
- EmailJS: Gửi OTP đến email người dùng qua template cấu hình sẵn.
- Firebase Admin SDK: Cập nhật mật khẩu trực tiếp qua `authAdmin.updateUser()` tại route `POST /api/auth/reset-password` (Fastify API, port 3001).
- Next.js API proxy route (`/api/auth/reset-password`): Chuyển tiếp request từ web sang Fastify backend tránh CORS.

Ẩn Dashboard Shell (Sidebar, Header, MobileNav) trên trang `/forgot-password` bằng cách bổ sung vào danh sách `hideShell` trong `layout.tsx` — hành vi nhất quán với trang `/login` và `/register`.

Tách CSS riêng (`forgot-password.css`) để tránh lỗi Hydration Mismatch khi dùng `<style>` tag inline trong Next.js `"use client"` component.
