...
> Cơ sở nghiệp vụ: `docs/SPEC.md` phiên bản 2.1  
> Phạm vi: Role-based architecture, brand isolation, label lifecycle, contact intelligence, onboarding, reporting, audit và auto-response safety
...
### 3.8 Contact-level context

Hệ thống không chỉ phân tích từng mention rời rạc. Sprint 2 cần bổ sung bối cảnh theo contact trong phạm vi brand:

- Gom các post/comment của cùng một contact trong cùng platform và brand.
- Tính sentiment history, topic history và tần suất đề cập.
- Tính influence score từ các tín hiệu tương tác như like, comment, share, reply, view hoặc follower nếu nguồn có.
- Tính spam score ở cấp content và contact.
- Tăng priority nếu contact có nhiều nội dung tiêu cực, influence cao và spam score thấp.
- Không boost priority nếu contact/content có spam score cao.
...
| Contact Intelligence | Gom lịch sử post/comment theo contact-brand, tính influence/spam/priority |
...
Contact Intelligence nằm ở lớp sau labeling/publish và trước các queue nghiệp vụ:

```text
Published Data
-> Contact Resolution
-> Contact Brand Aggregate
-> Influence + Spam Assessment
-> Priority Scoring
-> Crisis Queue / Lead Queue / Reports
```
...
| `Contact` | Người/tài khoản/tác giả được nhận diện trong phạm vi một nền tảng |
| `ContactIdentity` | Mapping định danh contact theo platform, URL, username hoặc ID nguồn |
| `ContactBrandProfile` | Trạng thái tổng hợp của contact đối với một brand |
| `ContactMentionAggregate` | Tổng hợp số lượng mention, sentiment distribution, topics, crisis count, lead intent |
| `InfluenceSignal` | Tín hiệu ảnh hưởng như like, comment, share, reply, view, follower nếu có |
| `SpamAssessment` | Đánh giá spam/bot/noise ở cấp content hoặc contact |
| `PriorityScore` | Điểm ưu tiên xử lý có xét sentiment history, influence, recency và spam |
...
| `contact_resolved` | Contact Resolution | AI Labeling/Spam Assessment | Đã gắn mention với contact trong phạm vi platform/brand nếu đủ tín hiệu |
...
| `contact_aggregated` | Contact Intelligence | Crisis/Lead/Reports | Contact-brand profile đã được cập nhật từ dữ liệu published |
...
### 7.5 Contact Intelligence processing

Contact Intelligence xử lý ở hai thời điểm:

1. **Pre-publish support**: hỗ trợ Admin nhận diện content/contact nghi spam trước khi publish.
2. **Post-publish aggregation**: cập nhật contact-brand profile từ dữ liệu đã publish để phục vụ crisis queue, lead queue và report.

Quy tắc định danh Sprint 2:

- Ưu tiên `platform_contact_id`, profile URL hoặc username ổn định trong cùng một platform.
- Display name chỉ là tín hiệu phụ, không đủ để merge contact.
- Không bắt buộc gom contact xuyên nền tảng trong Sprint 2.

Priority scoring ở mức khái niệm:

```text
contact_priority_score =
  base_crisis_score
  + negative_frequency_weight
  + influence_weight
  + recency_weight
  - spam_penalty
```
...
- `negative_ratio` cao làm tăng priority.
- `influence_score` cao làm tăng priority.
- Mention tiêu cực lặp lại trong thời gian ngắn làm tăng priority.
- `spam_score` cao làm giảm hoặc chặn priority boost.
- Priority score chỉ dùng để ưu tiên queue, không thay thế quyết định nghiệp vụ của người dùng.
...
| Contact Intelligence | Service/helper hiện có, `components/mentions`, `components/alerts`, `components/reports` |
...
| `/contacts` | Contact profile, contact-brand aggregate và contact priority theo quyền |
...
- Contact brand aggregates đã tính từ dữ liệu published.
...
| Brand Manager | Brand health, sentiment trend, negative topics, top risky contacts, lead summary, crisis summary, request history, auto-response status |
| Crisis Employee | Negative mentions handled, influential negative contacts, open crisis tasks, crisis topics, label requests submitted |
| Lead Employee | Leads handled, hot/warm/cold distribution, contact context for leads, conversion status, overdue leads |
...
| Contact merge/split hoặc contact identity update | System/Admin |
| Spam assessment thay đổi | System/Admin |
| Contact priority boost được áp dụng hoặc bị chặn | System |
...
- Contact spam score distribution.
- Contact priority boost count.
- Influential negative contacts count.
...
- Contact profile và aggregate chỉ trong brand của họ.
...
- Contact/content nghi spam trong pipeline kiểm duyệt.
- Audit/system data.
...
- Contact-brand profile liên quan đến item khủng hoảng của brand mình.
...
- Contact-brand context liên quan đến lead đó.
...
- Bổ sung contact identity contract trong phạm vi platform/brand.
- Gắn `contact_id` cho mention khi có tín hiệu đủ tin cậy.
- Tính contact-brand aggregate từ published mentions.
- Tính influence score từ engagement signals có sẵn.
- Tính spam score ở content-level và contact-level.
- Tính priority score và đẩy vào crisis/lead/report theo quyền.

### Phase 5 - Label correction workflow
...
### Phase 6 - Role-based dashboard/report

- Giữ route `/dashboard`, tách data widgets theo policy và role.
- Report dùng snapshot và active approved labels.
- Report bổ sung top risky contacts, influential negative contacts và spam/noise summary.
- Thêm empty state và onboarding theo vai trò.

### Phase 7 - Auto-response safety
...
- Hệ thống gom được nhiều post/comment của cùng contact trong cùng platform và brand.
- Contact có negative ratio cao, influence cao và spam score thấp được boost priority.
- Contact/content có spam score cao không được boost priority như contact thật.
- Brand users không xem được contact profile của brand khác.
...
|---|---|---|
| 2.1 | 2026-07-02 | Bổ sung Contact Intelligence: contact resolution, contact-brand aggregate, influence/spam assessment và priority scoring. |
