...
const newVi = {
  ...
  "header.fromHighlands": "từ Highland Coffee",
...
const newEn = {
  ...
  "header.fromHighlands": "from Highland Coffee",
...
// 1. Sidebar.tsx
translateFile(
  "d:/baitap/InsightFlow/InsightFlow/apps/web/src/components/layout/Sidebar.tsx",
  [
    ...
    {
      search: />💡 Báo cáo sẵn sàng</g,
      replace: '>{t("sidebar.reportReadyTitle")}<',
    },
    {
      search: />\s*AI hoàn tất tổng hợp dữ liệu ngày hôm qua\s*</g,
      replace: '>{t("sidebar.reportReadyDesc")}<',
    },
    { search: />\s*Xem báo cáo\s*</g, replace: '>{t("sidebar.viewReport")}<' },
    {
      search: />System Active 24\/7</g,
      replace: '>{t("sidebar.systemActive")}<',
    },
  ],
);
...
// 2. MobileNav.tsx
translateFile(
  "d:/baitap/InsightFlow/InsightFlow/apps/web/src/components/layout/MobileNav.tsx",
  [
    { search: /label: "Dashboard"/g, replace: 'label: "nav.dashboard"' },
    { search: /label: "Mentions"/g, replace: 'label: "nav.mentions"' },
    { search: /label: "Alerts"/g, replace: 'label: "nav.alerts"' },
...
// 3. Header.tsx
translateFile(
  "d:/baitap/InsightFlow/InsightFlow/apps/web/src/components/layout/Header.tsx",
  [
    {
      search: /placeholder="Tìm kiếm mention, bài viết\.\.\."/g,
      replace: 'placeholder={t("header.searchPlaceholder")}',
    },
    { search: />\s*Giả lập:\s*</g, replace: '>{t("header.simulation")}<' },
    {
      search: />Thông báo mới \(5\)</g,
      replace: '>{t("header.newNotifications")}<',
    },
    { search: />🔴 Hot Lead mới</g, replace: '>{t("header.newLead")}<' },
    {
      search: />\s*từ Highlands Coffee\s*</g,
      replace: '>{t("header.fromHighlands")}<',
    },
    {
      search: />🚨 Cảnh báo khủng hoảng</g,
      replace: '>{t("header.crisisAlert")}<',
    },
    {
      search: />\s*Spike detected: 3\.2x\s*</g,
      replace: '>{t("header.spikeDetected")}<',
    },
