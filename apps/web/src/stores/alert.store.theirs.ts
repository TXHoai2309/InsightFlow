      ...
      try {
        const loadAlerts = async () => {
          try {
            ...
            const message = error instanceof Error ? error.message : "Lß╗ùi ─æß╗ông bß╗Ö Supabase";
        ...
        const intervalId = setInterval(loadAlerts, 30 * 60 * 1000);
        activeUnsubscribe = () => clearInterval(intervalId);
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "Kh├┤ng thß╗â khß╗ƒi tß║ío ─æß╗ông bß╗Ö";
        set({ error: message, isLoading: false });
    ...
    updateAlertStatus: async (id, newStatus, profile, attempt) => {
      ...
      set((state) => {
        const nextRawAlerts = state.rawAlerts.map((alert) => {
          if (alert.id === id) {
            // When restoring to 'new' (Kh├┤i phß╗Ñc), clear history so the alert starts fresh
            if (newStatus === "new") {
              return {
                ...alert,
                status: newStatus,
