...
export function Dashboard({
  ...
  const metricsRowData = {
    ...
    unprocessedContacts: { 
      value: filteredLeadsForStats.filter(l => l.status === 'new').length
    },
    monitoredCrises: { 
  ...
  const newLeads = filteredLeadsForStats.filter(l => l.status === 'new');
    ...
    <div className="space-y-6 md:space-y-8 max-w-[1600px] mx-auto pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <LeadFunnelCard 
            funnelData={{
              new: filteredLeadsForStats.length,
              qualified: filteredLeadsForStats.filter(l => ['processing', 'completed'].includes(l.status)).length,
              contacted: filteredLeadsForStats.filter(l => ['processing', 'completed'].includes(l.status)).length,
              negotiating: filteredLeadsForStats.filter(l => ['completed'].includes(l.status)).length,
              won: filteredLeadsForStats.filter(l => l.status === 'completed').length
            }}
          />
        </div>
      </div>
