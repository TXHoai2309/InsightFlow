...
const PRAISE_COLORS = ["#22c55e", "#16a34a", "#4ade80", "#86efac", "#bbf7d0"];

interface TopPraisedTopicsChartProps {
  mentions: Mention[];
}

export function TopPraisedTopicsChart({ mentions }: TopPraisedTopicsChartProps) {
  const data = useMemo(() => {
    const pos = mentions.filter(m => m.sentiment === "positive");
    const total = pos.length || 1;
    const counts: Record<string, number> = {};
    pos.forEach(m => { const t = m.topic || "other"; counts[t] = (counts[t] || 0) + 1; });
  ...
  return (
    <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-surface)] p-5 shadow-sm h-full">
      <h3 className="text-[15px] font-bold text-[var(--color-text-primary)] mb-1">📈 Top chủ đề được khen</h3>
      <p className="text-[12px] text-[var(--color-text-muted)] mb-4">5 chủ đề nhận nhiều phản hồi tích cực nhất</p>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} layout="vertical" margin={{ top: 0, right: 30, left: 5, bottom: 0 }} barSize={16}>
              ...
              <Tooltip
                ...
                formatter={(value: number, _name: string, props: { payload?: { pct: number } }) => [`${value} (${props?.payload?.pct ?? 0}%)`, "Lượt khen"]}
                labelStyle={{ fontSize: 13, fontWeight: 700, color: "var(--color-text-primary)" }}
                itemStyle={{ fontSize: 12 }}
              />
              <Bar dataKey="value" radius={[0, 6, 6, 0]} label={{ position: "right", fontSize: 11, fill: "#22c55e", formatter: (v: number) => `${Math.round((v / (data.reduce((s, d) => s + d.value, 0) || 1)) * 100)}%` }}>
                {data.map((_, idx) => <Cell key={idx} fill={PRAISE_COLORS[idx % PRAISE_COLORS.length]} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        )}
