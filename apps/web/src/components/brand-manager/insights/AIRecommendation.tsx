  ...
  return (
    <div className="rounded-2xl border border-[var(--color-brand)]/20 bg-gradient-to-r from-[var(--color-brand-subtle)] via-white to-red-50 p-6 shadow-sm">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h3 className="text-[17px] font-bold text-[var(--color-text-primary)]">{t("bm.aiRec.title", "AI Khuyến nghị hành động")}</h3>
          <p className="text-[12px] text-[var(--color-text-muted)]">{t("bm.aiRec.subtitle", "Tổng hợp từ phân tích tiềm năng & khủng hoảng")}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="rounded-xl border border-green-200 bg-white p-4">
          <h4 className="text-[14px] font-bold text-green-700 mb-3 flex items-center gap-2">
            ...
            {t("bm.aiRec.opportunity", "Nên tận dụng ngay")}
          </h4>
          {opportunities.length === 0 ? (
            <p className="text-[12px] text-[var(--color-text-muted)] italic">{t("bm.aiRec.noOpp", "Chưa phát hiện cơ hội đáng kể")}</p>
          ) : (
              ...
              {capitalize(opportunities).map((item, i) => (
                <li key={i} className="flex items-start gap-2 text-[13px]">
                  ...
                  <span className="text-[var(--color-text-primary)]">
                    <strong>{item}</strong> — {t("bm.aiRec.oppAction", "đang tăng bất thường, nên đẩy truyền thông ngay")}
                  </span>
                </li>
              ))}
            </ul>
        ...
        <div className="rounded-xl border border-red-200 bg-white p-4">
          <h4 className="text-[14px] font-bold text-red-700 mb-3 flex items-center gap-2">
            ...
            {t("bm.aiRec.crisis", "Cần xử lý khẩn")}
          </h4>
          {crisisSignals.length === 0 ? (
            <p className="text-[12px] text-[var(--color-text-muted)] italic">{t("bm.aiRec.noCrisis", "Chưa phát hiện rủi ro cần xử lý")}</p>
          ) : (
            <ul className="space-y-2">
              {handleList.map((item, i) => (
                <li key={i} className="flex items-start gap-2 text-[13px]">
                  ...
                  <span className="text-[var(--color-text-primary)]">
                    <strong>{item}</strong> — {t("bm.aiRec.crisisAction", "tín hiệu tiêu cực đang leo thang, cần phân công xử lý")}
                  </span>
                </li>
              ))}
            </ul>
      ...
      {opportunities.length === 0 && crisisSignals.length === 0 && (
        <div className="mt-4 text-center py-4">
          ...
          <p className="text-[13px] text-[var(--color-text-muted)]">{t("bm.aiRec.noData", "AI chưa có đủ tín hiệu để đưa ra khuyến nghị trong khoảng thời gian này")}</p>
        </div>
      )}
    </div>
  );
