...
export function BrandManagerDashboard({
  ...
  const crisisAlertKpi = useMemo(() => {
    ...
    const scopedAlerts = filterOperationalAlerts(baseAlerts, {
      profile,
  ...
  }, [isDemo, crisisAlerts, filters.workspace_id, filters.platform, profile]);

  const leadOperationalMetrics = useMemo(() => {
    const baseLeads = isDemo && leads.length === 0 ? DEMO_MOCK_LEADS : leads;
    const scopedLeads = filterOperationalLeads(baseLeads, {
      profile,
      workspaceId: filters.workspace_id,
      platform: filters.platform,
    });
