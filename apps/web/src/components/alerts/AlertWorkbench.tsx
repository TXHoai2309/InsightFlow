        ...
        <div className={`grid w-full items-start gap-y-[1vh] ${isPanelOpen ? "min-[1100px]:grid-cols-[32%_minmax(0,1fr)] min-[1100px]:gap-x-[0.75%]" : "grid-cols-1"}`}>
          <main data-tour="alerts-queue-list" className="min-w-0 scroll-mt-24 self-start rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-surface)] shadow-sm">
            ...
            <div className="space-y-[2.5%] p-[3%]">
              {props.isLoading && props.pageAlerts.length === 0 ? [0, 1, 2].map((item) => <div key={item} className="h-[18vh] animate-pulse rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-surface-raised)]" />) : props.pageAlerts.length === 0 ? <div className="flex min-h-[45vh] flex-col items-center justify-center rounded-lg border border-dashed border-[var(--color-border)] p-[8%] text-center"><p className="text-sm font-black text-[var(--color-text-primary)]">Không có cảnh báo phù hợp</p><p className="mt-1 text-xs text-[var(--color-text-secondary)]">Thử chọn trạng thái hoặc điều chỉnh bộ lọc.</p></div> : props.pageAlerts.map((alert) => <AlertWorkbenchRow key={alert.id} alert={alert} selected={props.selectedAlertId === alert.id} onSelect={props.onSelectAlert} getResolverName={props.getResolverName} />)}
            </div>
...
function FilterPanel(props: AlertWorkbenchProps) {
  return <div className="grid gap-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-surface)] p-3 sm:grid-cols-2 xl:grid-cols-6">
    ...
    <button type="button" onClick={() => props.onMineOnlyChange(!props.showMineOnly)} className={`rounded-lg border px-3 py-2 text-sm font-bold ${props.showMineOnly ? "border-[var(--color-brand)] bg-[var(--color-brand-subtle)] text-[var(--color-brand)]" : "border-[var(--color-border)] text-[var(--color-text-secondary)]"}`}>Chỉ việc của tôi</button>
