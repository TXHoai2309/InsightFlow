  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto">
        ...
        {/* Headline */}
        <h1 className="text-[32px] md:text-[44px] leading-[1.2] tracking-tight font-[800] mb-4 max-w-4xl mx-auto" style={{ color: isDark ? "var(--color-text-primary)" : "#1a1a2e" }}>
          Tối ưu hóa hiệu suất truyền thông cho ngành <span style={{ background: "linear-gradient(90deg, #6D4CFF, #3B82F6)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>F&B</span>
        </h1>

        {/* Subtext */}
        <p className="text-[16px] md:text-[18px] leading-[1.6] font-normal max-w-2xl mx-auto mb-8" style={{ color: isDark ? "var(--color-text-muted)" : "#64748B" }}>
        ...
        <div className="flex flex-wrap justify-center items-center gap-3 text-[14px] md:text-[16px] font-medium">
          <div className="flex items-center gap-2">
            ...
            <span style={{ color: isDark ? "var(--color-text-muted)" : "#64748B" }}>Doanh nghiệp F&B</span>
          </div>
          <span className="text-[#CBD5E1] hidden sm:block">•</span>
          <div className="flex items-center gap-2">
            <span className="text-[#6D4CFF] font-bold text-[18px]">{kpiReview}M+</span>
            <span style={{ color: isDark ? "var(--color-text-muted)" : "#64748B" }}>Review phân tích</span>
          </div>
          <span className="text-[#CBD5E1] hidden sm:block">•</span>
          <div className="flex items-center gap-2">
            <span className="text-[#6D4CFF] font-bold text-[18px]">{kpiAccuracy}%</span>
            <span style={{ color: isDark ? "var(--color-text-muted)" : "#64748B" }}>Độ chính xác</span>
          </div>
        </div>
      </div>
    </section>
